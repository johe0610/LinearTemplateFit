...
# Analysis object: /WbWb_singlelepton/m_minimax_fine
# Timestamp: 07-08-2026 (09:26:45)
...
ax_yScale = 'log'
xLims = (0.0, 960.0)
yLims = (9.000000000000001e-09, 0.011000000000000001)

# TeX-friendly labels for the legend
labels = {
  'legend' : ([ r"user.johessle.mc15\_13TeV.410472.WbWb\_singlelepton.test\_topmass\_mppui", r"user.johessle.mc15\_13TeV.411053.WbWb\_singlelepton.test\_topmass\_mppui", r"user.johessle.mc15\_13TeV.411058.WbWb\_singlelepton.test\_topmass\_mppui" ], r""),
}
...
exec(open(prefix+'m_minimax_fine__data.py').read(), dataf)
...
for label, yvals in dataf['ratio0_yvals'].items():
    ...
    if styles[label]['ratio0_histstyle']: # plot as histogram
        xpos = dataf['xedges']['user.johessle.mc15_13TeV.410472.WbWb_singlelepton.test_topmass_mppui.yoda'] if styles[label]['ratio0_drawstyle'] else dataf['xpoints']['user.johessle.mc15_13TeV.410472.WbWb_singlelepton.test_topmass_mppui.yoda']
    ...
    tmp = ratio0_ax.errorbar(dataf['xpoints']['user.johessle.mc15_13TeV.410472.WbWb_singlelepton.test_topmass_mppui.yoda'], yvals,
    ...
    for varlabel in dataf['ratio0_variation_vals'].keys():
        if varlabel.startswith(label):
            ratio0_ax.plot(dataf['xedges']['user.johessle.mc15_13TeV.410472.WbWb_singlelepton.test_topmass_mppui.yoda'],
...
plt.savefig(os.path.join(plot_outdir, 'm_minimax_fine.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'm_minimax_fine.png'), format='PNG')
