...
# Analysis object: /WbWb_dilepton/phi_jet2_untagged
# Timestamp: 27-02-2025 (11:02:17)
...
labels = {
    'legend' : ([ r"S3beta\_Cluster\_WbWb\_dilepton\_170", r"S3beta\_Cluster\_WbWb\_dilepton\_160", r"S3beta\_Cluster\_WbWb\_dilepton\_165", r"S3beta\_Cluster\_WbWb\_dilepton\_175", r"S3beta\_Cluster\_WbWb\_dilepton\_180" ], r""),
}
...
exec(open(prefix+'phi_jet2_untagged__data.py').read(), dataf)
...
for label, yvals in dataf['ratio0_yvals'].items():
    ...
    if styles[label]['ratio0_histstyle']: # plot as histogram
        xpos = dataf['xedges']['S3beta_Cluster_WbWb_dilepton_170.yoda.gz'] if styles[label]['ratio0_drawstyle'] else dataf['xpoints']['S3beta_Cluster_WbWb_dilepton_170.yoda.gz']
    ...
    tmp = ratio0_ax.errorbar(dataf['xpoints']['S3beta_Cluster_WbWb_dilepton_170.yoda.gz'], yvals,
    ...
    for varlabel in dataf['ratio0_variation_vals'].keys():
        if varlabel.startswith(label):
            ratio0_ax.plot(dataf['xedges']['S3beta_Cluster_WbWb_dilepton_170.yoda.gz'],
...
plt.savefig(os.path.join(plot_outdir, 'phi_jet2_untagged.pdf'), format='PDF')
plt.savefig(os.path.join(plot_outdir, 'phi_jet2_untagged.png'), format='PNG')
